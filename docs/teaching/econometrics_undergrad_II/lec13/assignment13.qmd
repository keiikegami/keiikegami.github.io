...
::: {.callout-warning}
## よくあるバグ：$\beta$ を推定しようとして地獄を見る

**$\beta$ を推定対象に加えてはいけない。** 割引因子は選択データだけからは識別が難しく（$\beta$ と flow utility がもつれる）、`optim` の探索が発散したり、無意味な値（$\beta > 1$ や負）に飛んだりする。$\beta = 0.95$ は**固定**し、推定するのは $(\theta_1, RC)$ の2つだけ。これは実務の標準的な慣習でもある。
:::
...
$y(x)$ を経験 CCP から計算し、$x$ に回帰する（傾き=$\theta_1$、切片=$-RC$）。$\hat P = 0$ 対策に Laplace 平滑化、薄い状態対策に観測数 $\geq 30$ の状態だけ重み付き回帰。
