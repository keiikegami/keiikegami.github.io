...
$\text{low\_threshold}_{jt}$は「閾値1,000円以上（低い方）なら1」のダミー、$\text{coupon}_{jt}$は「クーポン配布ありなら1」のダミー、$\text{price}_{jt}$は月額料金（円）である。$\varepsilon_{ijt}$はType I極値分布。個人ごとの部分効用は正規分布に従う異質性を持つ（mixed logit）。
...
\alpha_i = \alpha + \sigma_\alpha z_{\alpha,i}
$$

$z_{\cdot,i}\sim N(0,1)$で、各回答者は8タスクすべてで同じ係数を使う（回答者内クラスター相関の源）。
...
`alpha_mean = 0.35`は「価格が百円動いたときの効用の変化」を表すよう設計している（講義ノートでは価格を千円単位で扱っていたが、今回は月額料金がもっと小さい金額なので百円単位にしてある）。`price / 100`を忘れると、価格の効用への効き方が3桁ずれてしまい、ほぼ全員が「どちらも入会しない」を選ぶ、あるいは逆にほぼ全員が価格を無視する、といった不自然なシェアになる。まず小さいNで`table(cbc_raw$choice3)`を見て、極端に偏っていないか確認する習慣をつけよ。
:::
...
属性間の相関がゼロに近いことを確認するのは「ランダム化が実装バグなく機能しているか」の最も基本的なチェックである。もし相関が大きく出たら、`generate_profiles`のロジックか、`sample`の引数を疑うこと。選択シェアについては、outsideが極端に大きい（例えば9割超）と、A/Bの比較に使える情報が乏しくなり、後のAMCE推定の精度が落ちる。適度にoutsideが選ばれている（2〜4割程度）のが健全な調査設計である。
:::

## 問4（10点）：long形式への変換

AMCE推定用に、各タスクをプランA行・プランB行に分解したlong形式のデータフレームを作る関数`to_long`を完成させよ。outside optionを選んだタスクは、A対Bの2値選択回帰からは除外すること。
...
## 問6（12点）：(ii) logit MLEで効用係数を推定し、月額料金のWTPをデルタ法CI付きで求める

第2回・第6回の自作MLEパターンで、`neg_loglik_cbc`を書いてoptimで最大化し、Hessianから共分散行列を得よ。そのうえで、**低閾値（1,000円）へのWTP**をデルタ法で信頼区間付きで求めよ（月額料金の単位＝百円であることに注意）。
...
「低閾値」×「クーポンあり」の交互作用項を含めてAMCEを再推定し、係数とSEを報告せよ。この交互作用が統計的にゼロと区別できるかどうかを判定し、2〜3文で解釈せよ（このDGPで交互作用が実際にどう設計されているかを思い出すこと）。
...
問5と同じ回帰について、`vcov = "hetero"`（通常のロバストSE、クラスターを無視）を指定した場合のSEを計算し、クラスターSEと並べた比較表を作れ。**なぜクラスターSEの方が大きくなるのか（あるいは通常のSEがなぜ縮むのか）を3〜4文で説明せよ**。
...
「独立な情報の単位は何か」を考えよ。500人×8タスク=4,000行だが、独立にランダムに変動しているのは本当に4,000個の別々の情報なのか、それとも実質500人分の情報なのか。回答者内で誤差が相関する理由（同じ人は毎回似た好みで選ぶ）を、この課題のDGPの言葉（`beta_coupon_i`などが回答者ごとに固定されて8タスクで使い回されること）で説明すると良い解答になる。
:::
