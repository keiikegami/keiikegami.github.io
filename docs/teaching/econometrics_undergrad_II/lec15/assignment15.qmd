...
そこで、設問1-1・1-2・1-3は「一定価格のときのブッキングカーブがどう見えるか」を体感するために行うが、**Part 2で使うデータは、次の設問1-4で作る「ほぼ一定だが小さくランダムに変動する価格」のデータに切り替える**。実務でも、まったく同じ1円単位の価格を60日間維持することは稀で、早期割引や曜日効果的な微調整が入るのが普通である。この設問1-4のデータの方が、設問1-1よりも実務のデータに近い。
:::

### 設問1-4（配点は上記10点に含む、必須）：価格にわずかな変動を持たせたDGP

設問1-1のシミュレーション関数を改造し、価格を「基準価格 $\bar p$ の $\pm 15\%$ の範囲で、5段階（$-15\%, -7.5\%, 0\%, +7.5\%, +15\%$）のいずれかを毎日ランダムに選ぶ」ように変更せよ。基準価格 $\bar p$ は設問2-4で求める最適固定価格に近い値（目安：90前後）を使ってよい。
...
simulate_varying_price_dgp <- function(seed, T_period, C_capacity, theta0, alpha, lambda_bar,
  ...
  for (t in T_period:1) {
    ...
    # 到着かつ購入が観測された客についてのみ、prices_obsとpurchase_obsにp・purchasedを追加する
    # （到着しなかった客・在庫が既にない期は記録しない、という設計にしてよい）。
    # 購入されたらc_remainを1減らす。
  }
...
### 設問2-2（15点）：$\hat\alpha$ でbackward inductionを解き、価格政策のヒートマップを描く
...
### 設問2-3（15点）：推定誤差のコスト — $\hat\alpha$ の政策 vs 真の $\alpha$ の政策
...
設問1-4のデータ生成から設問2-3の収益ロス計算までの一連の流れを**1つの関数にまとめ**、乱数シードを変えながら**200回繰り返せ**。得られた200個の $\hat\alpha$ と収益ロスについて、

- $\hat\alpha$ の標本分布（ヒストグラム）
- 収益ロスの分布（ヒストグラム）

を描き、平均・中央値・標準偏差を報告せよ。
...
for (m in 1:n_mc) {
  ...
  #    このmをスキップして次のmに進む。alpha_hat_vec[m], revenue_loss_vec[m]はNAのままにする）

  # 3. MLE推定：設問2-1の方法でtheta0_hat, alpha_hatを求める
  #    （alpha_hatが0以下・非有限・極端に大きい場合も「推定失敗」としてスキップする）

  # 4. 推定alphaでDPを解く：設問2-2のsolve_dp()を呼び出す
...
配点は Part 1（30点）+ Part 2（70点）の合計100点。不完全な解答にも、考え方と実装の進捗に応じて部分点を与える。
