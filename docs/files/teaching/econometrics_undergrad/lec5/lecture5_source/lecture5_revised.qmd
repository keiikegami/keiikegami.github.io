...
![Geometric interpretation of OLS projection.](assets/fwl_projection_geometry.svg){fig-alt="A geometric diagram where Y is projected onto a predicted plane, with Y-hat on the plane and the residual shown as the perpendicular difference." width="78%"}
...
$$
\hat a=
\frac{\sum_{i=1}^n \tilde X_{1i}\tilde X_{2i}}
{\sum_{i=1}^n \tilde X_{2i}^2}
$$
...
\tilde X_{1i}
-
\frac{\sum_{j=1}^n \tilde X_{1j}\tilde X_{2j}}
{\sum_{j=1}^n \tilde X_{2j}^2}
\tilde X_{2i}
$$
...
$$
\hat c=
\frac{\sum_{i=1}^n \tilde X_{2i}\tilde Y_i}
{\sum_{i=1}^n \tilde X_{2i}^2}
$$
...
\tilde Y_i
-
\frac{\sum_{j=1}^n \tilde X_{2j}\tilde Y_j}
{\sum_{j=1}^n \tilde X_{2j}^2}
\tilde X_{2i}
$$
...
\sum_{i=1}^n v_i r_i
=
\sum_{i=1}^n \tilde X_{1i}\tilde Y_i
-
\frac{
\left(\sum_{i=1}^n \tilde X_{1i}\tilde X_{2i}\right)
\left(\sum_{i=1}^n \tilde X_{2i}\tilde Y_i\right)
...
\sum_{i=1}^n v_i^2
=
\sum_{i=1}^n \tilde X_{1i}^2
-
\frac{
\left(\sum_{i=1}^n \tilde X_{1i}\tilde X_{2i}\right)^2
}{
\sum_{i=1}^n \tilde X_{2i}^2
}
$$
...
::: {.callout-important title="FWL定理の読み方"}
重回帰
$$
Y_i=\beta_0+\beta_1X_{1i}+\beta_2X_{2i}+u_i
$$
における $X_1$ の係数 $\hat\beta_1$ は、

1. $X_1$ から $X_2$ で説明できる部分を取り除く
2. $Y$ から $X_2$ で説明できる部分を取り除く
3. その残りどうしを単回帰する
...
n_fwl <- 40
x2_fwl <- seq(-2, 2, length.out = n_fwl)
x1_fwl <- 0.8 * x2_fwl + rnorm(n_fwl, sd = 0.6)
y_fwl  <- 1.5 * x1_fwl + 0.9 * x2_fwl + rnorm(n_fwl, sd = 0.7)

fit_x1_on_x2 <- lm(x1_fwl ~ x2_fwl)
fit_y_on_x2  <- lm(y_fwl ~ x2_fwl)

v_fwl <- resid(fit_x1_on_x2)
r_fwl <- resid(fit_y_on_x2)
...
plot(x2_fwl, x1_fwl,
     xlab = "X2",
     ylab = "X1",
     main = "1. Predict X1 from X2")
abline(fit_x1_on_x2, lwd = 2)
segments(x2_fwl[show_i],
         fitted(fit_x1_on_x2)[show_i],
         x2_fwl[show_i],
         x1_fwl[show_i],
         lty = 2)
...
plot(x2_fwl, y_fwl,
     xlab = "X2",
     ylab = "Y",
     main = "2. Predict Y from X2")
abline(fit_y_on_x2, lwd = 2)
segments(x2_fwl[show_i],
         fitted(fit_y_on_x2)[show_i],
         x2_fwl[show_i],
         y_fwl[show_i],
         lty = 2)

plot(v_fwl, r_fwl,
     xlab = "Residualized X1 (v)",
     ylab = "Residualized Y (r)",
     main = "3. Regress residuals")
abline(lm(r_fwl ~ v_fwl), lwd = 2)
...
X_{1i}
=
a_0+a_2X_{2i}+\cdots+a_KX_{Ki}+v_i
$$
...
Y_i
=
c_0+c_2X_{2i}+\cdots+c_KX_{Ki}+r_i
$$
