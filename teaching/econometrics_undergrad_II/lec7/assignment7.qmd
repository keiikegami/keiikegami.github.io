...
- **配信$D_i \in \{0,1\}$**：ランダムに50%の確率で配信する。
...
- 配信 $D_i$：50%の確率でランダムに割り当てる（`rbinom`）。
...
$$
Y_i = \max(\text{outcome\_base}_i + U_i \times 800,\ 0)
$$
...
使用するかどうかは能力に依存する自己選択である。能力の高い顧客ほどクーポンを使い、かつ能力の高い顧客はクーポンなしでも（`outcome_base`が高いため）購買額が高い。この2つの効果が混ざって、単純比較はTOTを大幅に（2倍以上）過大評価する。倍率がだいたい2〜3倍程度になっていれば正しい実装である。
:::
...
3. 処置ラベル$D$を2,000回シャッフルし、ランダム化推論のp値を計算せよ。t検定のp値と比較せよ。
...
$N=10,000$で2,000回のシャッフルを行うと、この課題のPCでは数秒〜十数秒程度かかることがある。シャッフルのたびに`coupon_df$Y`全体を並べ替える必要はなく、`sample(coupon_df$D)`でラベルだけをシャッフルすれば十分に速い。もし遅すぎる場合は`n_perm`を1000に減らしても、結論（t検定とほぼ一致すること）は変わらない。
:::

### Q7（12点）：TOT推定：配信をIVにした2SLS

1. Wald推定量（TOT = ITT ÷ 使用率）を計算せよ。
...
2. 分散削減率とSE削減率を計算し、$\text{corr}(\text{pre\_period}, Y)^2$の理論値と比較せよ。
...
- Q9のpeekingシミュレーションで、覗く回数が増えるほどType I errorが単調に増加していることを確認したか。
