...
```{r}
typewriter_interval <- function(n) {
  m <- floor(log2(n))
  k <- n - 2^m
  c(left = k / 2^m, right = (k + 1) / 2^m, m = m, k = k)
}

typewriter_value <- function(n, omega) {
  interval <- typewriter_interval(n)
  as.numeric(omega >= interval["left"] & omega < interval["right"])
}
...
```{r}
n_grid <- 1:100
lp_moment <- 1 / n_grid^2
tail_prob <- 1 / n_grid^2
markov_bound <- lp_moment / 0.5

plot(n_grid, tail_prob, type = "l", lwd = 2,
     xlab = "n", ylab = "probability / bound",
     main = "L^1 control implies probability control")
lines(n_grid, markov_bound, lty = 2, lwd = 2)
legend("topright",
       legend = c("P(|X_n| > 0.5)", "E|X_n| / 0.5"),
