...
#| message: false
#| warning: false
#| fig-cap: "一致性のイメージ：サンプルを1つずつ追加したときの OLS 推定量の軌跡"
#| fig-width: 8
#| fig-height: 5.5
...
x_path <- rnorm(n_path)
u_path <- rnorm(n_path)
y_path <- beta0 + beta1 * x_path + u_path

n_seq <- 3:n_path
b_path <- numeric(length(n_seq))

for (j in seq_along(n_seq)) {
  nn <- n_seq[j]
  b_path[j] <- coef(lm(y_path[1:nn] ~ x_path[1:nn]))[2]
}

plot(n_seq, b_path, type = "l", lwd = 2,
     xlab = "Sample size n",
     ylab = expression(hat(beta)[1]^{(n)}),
     main = "Consistency: one sample path")
abline(h = beta1, lty = 2, lwd = 2)
text(170, beta1 + 0.07,
     labels = bquote("true value: " * beta[1] == .(beta1)),
     pos = 4)
```
...
#| message: false
#| warning: false
#| fig-cap: "一致性のイメージ：多数の反復でみた OLS 推定量の軌跡"
#| fig-width: 8
#| fig-height: 5.8
...
n_path <- 200
n_min_plot <- 10
n_seq <- n_min_plot:n_path
paths <- matrix(NA_real_, nrow = R_path, ncol = length(n_seq))

for (r in 1:R_path) {
  x_now <- rnorm(n_path)
  u_now <- rnorm(n_path)
  y_now <- beta0 + beta1 * x_now + u_now
  
  for (j in seq_along(n_seq)) {
    nn <- n_seq[j]
    paths[r, j] <- coef(lm(y_now[1:nn] ~ x_now[1:nn]))[2]
  }
}

mean_path <- colMeans(paths)

plot_limits <- quantile(as.vector(paths), probs = c(0.005, 0.995), na.rm = TRUE)
y_lim <- range(plot_limits, beta1)
y_lim <- y_lim + c(-1, 1) * 0.08 * diff(y_lim)
paths_for_plot <- paths
paths_for_plot[paths_for_plot < y_lim[1] | paths_for_plot > y_lim[2]] <- NA

plot(range(n_seq), y_lim, type = "n",
     xlab = "Sample size n",
     ylab = expression(hat(beta)[1]^{(n)}),
     main = "Consistency: many sample paths")

for (r in 1:R_path) {
  lines(n_seq, paths_for_plot[r, ], col = rgb(0.2, 0.5, 0.9, 0.15), lwd = 1)
}
lines(n_seq, mean_path, lwd = 3, col = "blue4")
abline(h = beta1, lty = 2, lwd = 2, col = "blue4")
legend("topright",
       legend = c("replication", "mean", "true value"),
       lty = c(1, 1, 2),
       lwd = c(1, 3, 2),
       col = c(rgb(0.2, 0.5, 0.9, 0.35), "blue4", "blue4"),
       bty = "n")
```
...
#| message: false
#| warning: false
#| fig-cap: "漸近正規性のイメージ：標本サイズが大きくなるにつれて標準化した推定誤差の分布が正規分布に近づく"
#| fig-width: 10
#| fig-height: 7
...
R_asym <- 2000
n_grid <- c(5, 20, 80, 400)
z_list <- vector("list", length(n_grid))

for (k in seq_along(n_grid)) {
  n_asym <- n_grid[k]
  z_now <- numeric(R_asym)
  
  for (r in 1:R_asym) {
    x_now <- rnorm(n_asym)
    u_now <- rexp(n_asym, rate = 1) - 1
    y_now <- beta0 + beta1 * x_now + u_now
    b_hat_now <- coef(lm(y_now ~ x_now))[2]
    z_now[r] <- sqrt(n_asym) * (b_hat_now - beta1)
  }
  
  z_list[[k]] <- as.numeric(scale(z_now))
}

par(mfrow = c(2, 2), mar = c(4, 4, 3, 1))

for (k in seq_along(n_grid)) {
  hist(z_list[[k]], breaks = 35, freq = FALSE,
       xlim = c(-4, 4), ylim = c(0, 0.45),
       main = paste0("n = ", n_grid[k]),
       xlab = "standardized estimation error")
